...
public class AddSpawnPointPlayerCommand implements CommandExecutor
{
	@Override
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
	{
		...
		else
		{
						...
						if (!map.getPlayerSpawnPoints().contains(player.getLocation().getBlockX() + "," + player.getLocation().subtract(0, 1, 0).getBlockY() + "," + player.getLocation().getBlockZ()))
						{
							...
							if (!map.getZombieSpawnPoints().contains(player.getLocation().getBlockX() + "," + player.getLocation().subtract(0, 1, 0).getBlockY() + "," + player.getLocation().getBlockZ()))
							{
								...
								if (!map.isEnabled())
								{
									...
									map.addPlayerSpawnLocation(location, block);
									...
									{
										// notify the sender
										sender.sendMessage(ChatColor.GREEN + "Added Player spawn point for map: "
											+ ChatColor.BOLD
											+ args[0]
											+ ChatColor.RESET
											+ ""
								...
								{
									// notify the sender
									sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + args[0] + ChatColor.RESET + "" + ChatColor.RED + " has to be disabled to add a Player spawn point");
								}
			...
			{
				// notify the sender
				sender.sendMessage(ChatColor.RED + "Correct usage: /z_addspawnpoint_player <map name>");
			}
		}
